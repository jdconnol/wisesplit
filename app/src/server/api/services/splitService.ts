...
import { sendExpensePushNotification, sendExpenseEmailNotification } from './notificationService';
...
export async function createExpense(
  {
    ...
    cronExpression,
    idempotencyKey,
  }: CreateExpense & { cronExpression?: string; idempotencyKey?: string },
  currentUserId: number,
  conversionFromParams?: CreateExpense,
) {
  // Idempotency: a retried request with the same key returns the original expense
  // instead of creating a duplicate.
  if (idempotencyKey) {
    const existing = await db.idempotencyRecord.findUnique({
      where: { userId_idempotencyKey: { userId: currentUserId, idempotencyKey } },
    });
    if (existing) {
      const prior = await db.expense.findUnique({ where: { id: existing.resourceId } });
      if (prior) {
        return prior;
      }
    }
  ...
  // Pre-generate UUID when recurring OR idempotent, so the id is known inside the
  // transaction (needed to tie the IdempotencyRecord to the expense atomically).
  let expenseId: string | undefined = undefined;
  let jobId: bigint | undefined = undefined;

  if (cronExpression || idempotencyKey) {
  ...
  // Record the idempotency key in the SAME transaction as the expense, so the
  // unique (userId, idempotencyKey) constraint makes a concurrent retry fail atomically.
  if (idempotencyKey && expenseId) {
    operations.push(
...
export async function deleteExpense(expenseId: string, deletedBy: number) {
  ...
  sendExpenseEmailNotification(expenseId).catch(console.error);
}
