...
import { sendExpenseNotificationEmail } from '~/server/mailer';
...
/**
 * Email counterpart to {@link sendExpensePushNotification}. Emails every participant
 * (other than the person who performed the action) who has an email address.
 * No-ops gracefully when SMTP is not configured.
 */
export async function sendExpenseEmailNotification(expenseId: string) {
  const expense = await db.expense.findUnique({
    where: { id: expenseId },
    select: {
      paidBy: true,
      ...
      conversionTo: { select: { currency: true, amount: true } },
      expenseParticipants: {
        select: { userId: true, amount: true, user: { select: { name: true, email: true } } },
      },
    },
  });
  ...
  const actorId = expense.deletedBy ?? expense.addedBy;
  const recipients = expense.expenseParticipants.filter(
    (p) => p.userId !== actorId && 0n !== p.amount && p.user?.email,
  );
  ...
  if (expense.deletedBy) {
    title = getUserDisplayName(expense.deletedByUser);
    message = `Deleted ${expense.name}`;
  } else if (expense.updatedByUser) {
    title = getUserDisplayName(expense.updatedByUser);
    message = `Updated ${expense.name} ${amount}`;
  } else if (expense.splitType === SplitType.CURRENCY_CONVERSION && expense.conversionTo) {
    const toAmount = formatAmount(expense.conversionTo.currency, expense.conversionTo.amount);
    message = `${payer} converted ${amount} → ${toAmount}`;
  } else if (expense.splitType === SplitType.SETTLEMENT) {
    message = `${payer} settled up ${amount}`;
  } else {
    message = `${payer} paid ${amount} for ${expense.name}`;
  }
